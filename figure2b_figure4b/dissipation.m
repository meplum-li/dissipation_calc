...
A = pi^2/6*(-T + diag(sum(T,2)));%所有电极之间的
id_dissipation_temper = false(Lattice.num_dissipation,1);%虚拟电极中，升温的电极
id_dissipation_temper(randperm(Lattice.num_dissipation, round(Stochastic.dissipation.temper* Lattice.num_dissipation))) = true;%虚拟电极中，升温的电极
id_temper = logical([zeros(6,1);id_dissipation_temper]);%所有电极中，升温的电极
A_tilde = A(id_temper, id_temper);%升温电极之间的
Ti_square = A_tilde\ (energy_current_p(id_temper)-T_bg^2*sum( A(id_temper, ~id_temper), 2 ));%T^2，升温电极的温度
Ti = zeros(Lattice.num_dissipation,1);
Ti(id_dissipation_temper) = (sqrt(Ti_square)-T_bg)/T_bg; %T，所有虚拟电极的温度
%%% 记录能流、以及虚拟电极温度的空间分布
